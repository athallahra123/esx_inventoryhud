...
--RegisterKeyMapping('z1o', '(A) Buka kantong / inventory', 'keyboard', 'F2')
...
RegisterCommand("z1o", function(source, args)
    local tas = openInventory()
    local tas = false
...
-- FIX BAG DALAM INVENTORY
--[[RegisterCommand("fixinventory", function(source, args)
	closeInventory()
end)]]

RegisterNetEvent('fixinventorybos')
AddEventHandler('fixinventorybos', function()
    closeInventory()
end)
...
RegisterNetEvent("esx_inventoryhud:AnimTrunk")
AddEventHandler("esx_inventoryhud:AnimTrunk" , function(data)
    AnimTrunk = true
    if AnimTrunk then
...
RegisterNUICallback("GetNearPlayers", function(data, cb)
    ...
    if not foundPlayers then
            exports['mythic_notify']:SendAlert('error', 'Tidak ada player didekat mu!')
        else
            SendNUIMessage(
                {
                    action = "nearPlayers",
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if foundPlayer then
			...
			TriggerEvent("esx:giveAnimation", playerId)
            Wait(250)
            ...
            exports['mythic_notify']:SendAlert('error', 'Tidak player didekat mu!')
        end
        cb("ok")
    end
)
                ...
                if fastWeapons[1] ~= nil then
                    if string.find(fastWeapons[1],"WEAPON_") then
                        if GetSelectedPedWeapon(GetPlayerPed(-1)) == GetHashKey(fastWeapons[1]) then
                            ...
                            TriggerEvent('esx_inventoryhud:refreshWeapon')
                        else
