...
function refreshPropertyGangInventory()
    ESX.TriggerServerCallback(
        "esx_inventoryhud:getGangInventory",
        function(inventory)
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            TriggerServerEvent("esx_gangjob:putItem", data.item.type, data.item.name, count)
        end

        Wait(150)
        refreshPropertyGangInventory()
        Wait(150)
        loadPlayerInventory()
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            TriggerServerEvent("esx_gangjob:getItem", data.item.type, data.item.name, tonumber(data.number))
        end
